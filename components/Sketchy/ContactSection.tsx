...
export function ContactSection({ data }: ContactProps) {
  const events = [
    { title: data.events.ceremony.title, time: data.events.ceremony.time },
    { title: data.events.reception.title, time: data.events.reception.time },
  ]
      ...
      <div className="max-w-[1000px] mx-auto text-center">
        ...
        <div className="flex flex-col md:flex-row justify-center gap-12 md:gap-24 mb-16">
          <div className="flex-1 text-center md:text-right">
            ...
            <p className="font-serif text-2xl md:text-3xl mb-2">{data.events.venue.name}</p>
            <p className="font-sans text-gray-400 font-light max-w-xs ml-auto mr-auto md:mr-0">{data.events.venue.address}</p>
          </div>
          ...
          <div className="flex-1 text-center md:text-left flex flex-col justify-center">
            {events.map((e) => (
              <div key={e.title} className="mb-6 last:mb-0">
