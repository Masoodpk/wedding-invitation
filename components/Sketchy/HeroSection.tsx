...
export function HeroSectionSketchy({ data }: HeroProps) {
  ...
  const ymd = parseYmd(data.wedding.dateTimeLocal)
  const dateText = ymd ? `${ymd.day} . ${ymd.month} . ${ymd.year}` : ''
      ...
      <div className="max-w-[1200px] w-full grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
        <div className="order-2 md:order-1 flex flex-col items-center md:items-start text-center md:text-left">
          ...
          <div className="relative mt-4">
            <p className="font-script text-[clamp(1.5rem,4vw,2.5rem)] text-gray-700">...совместные мечты</p>
          ...
          <div className="mt-12 flex flex-col items-center md:items-start gap-4 font-serif tracking-[0.2em] text-[#1A1A1A]">
            <p className="text-xl">{dateText}</p>
            <p className="text-sm opacity-60 uppercase">{data.events.venue.name}</p>
          </div>
        </div>

        <div className="order-1 md:order-2 relative">
          <div className="aspect-[4/5] bg-gray-200 border-[8px] border-white shadow-xl transform rotate-2 overflow-hidden">
            <img src={data.wedding.heroImageUrl} alt="Couple" className="w-full h-full object-cover grayscale" />
          </div>
