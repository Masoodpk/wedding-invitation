    ...
    <div className="fixed inset-0 z-[100] bg-[#F5F1E9] flex flex-col items-center justify-center p-4 text-center overflow-y-auto overflow-x-hidden">
      <div className="relative w-full max-w-[350px] sm:max-w-[400px] mb-8 group mt-8">
        <div className="aspect-[3/4] max-h-[40vh] mx-auto overflow-hidden grayscale contrast-125 border-[3px] border-[#1A1A1A] transform -rotate-1 transition-transform group-hover:rotate-0 duration-500">
          <img 
            src={data.details.heroImage || "https://images.unsplash.com/photo-1511285560929-80b456fea0bc?q=80&w=2069&auto=format&fit=crop"} 
            alt="Couple" 
            className="w-full h-full object-cover"
          />
        </div>
        {/* Sketchy Circle Overlay */}
      ...
      <div className="mb-8">
        ...
        <p className="font-script text-[clamp(1.2rem,3vw,2.5rem)] text-[#C62828] -mt-2">
      ...
      >
        <span className="relative z-10 font-serif text-base md:text-lg tracking-[0.2em] text-[#1A1A1A] uppercase">
          Open
        </span>
        {/* Sketchy Border Button */}
        <svg className="absolute inset-0 w-full h-full pointer-events-none" viewBox="0 0 200 60" preserveAspectRatio="none">
          <path d="M5,5 Q100,2 195,5 Q198,30 195,55 Q100,58 5,55 Q2,30 5,5 Z" 
                fill="none" stroke="#1A1A1A" strokeWidth="2"
                className="group-hover:stroke-[#C62828] transition-colors" />
        </svg>
      </button>
      
      <div className="mt-8 text-sm tracking-widest text-gray-500 font-serif uppercase animate-pulse">
        Scroll to explore
      </div>
    </div>
  )
